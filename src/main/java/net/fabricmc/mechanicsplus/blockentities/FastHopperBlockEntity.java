...
import net.fabricmc.mechanicsplus.blocks.FastHopperBlock;
import net.fabricmc.mechanicsplus.screens.FastHopperScreenHandler;
...
public class FastHopperBlockEntity extends LootableContainerBlockEntity implements Hopper, Tickable {
 ...
 private static boolean isInventoryEmpty(Inventory inv, Direction facing) {
  ...
  private static ItemStack transfer(Inventory from, Inventory to, ItemStack stack, int slot, Direction direction) {
    ...
    if (canInsert(to, stack, slot, direction)) {
      ...
      if (bl) {
        if (bl2 && to instanceof FastHopperBlockEntity) {
          FastHopperBlockEntity hopperBlockEntity = (FastHopperBlockEntity)to;
          if (!hopperBlockEntity.isDisabled()) {
            int k = 0;
            if (from instanceof FastHopperBlockEntity) {
              FastHopperBlockEntity hopperBlockEntity2 = (FastHopperBlockEntity)from;
  ...
  private Inventory getOutputInventory() {
    Direction direction = (Direction)this.getCachedState().get(FastHopperBlock.FACING);
