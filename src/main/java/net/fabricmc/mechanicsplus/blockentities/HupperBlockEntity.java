...
import net.fabricmc.mechanicsplus.blocks.HupperBlock;
import net.fabricmc.mechanicsplus.screens.HupperScreenHandler;
...
public class HupperBlockEntity  extends LootableContainerBlockEntity implements Hopper, Tickable {
  ...
  public HupperBlockEntity() {
    super(MechanicsPlusMod.HUPPER_ENTITY);
  ...
  private boolean insertAndExtract(Supplier<Boolean> extractMethod) {
    if (this.world != null && !this.world.isClient) {
      if (!this.needsCooldown() && (Boolean)this.getCachedState().get(HupperBlock.ENABLED)) {
        boolean bl = false;
        if (!this.isEmpty()) {
          bl = this.insert();
        }
  ...
  private boolean insert() {
    ...
    if (inventory == null) {
      ...
      Direction direction = ((Direction)this.getCachedState().get(HupperBlock.FACING)).getOpposite();
    ...
    if (canInsert(to, stack, slot, direction)) {
      ...
      if (bl) {
        if (bl2 && to instanceof HupperBlockEntity) {
          HupperBlockEntity hopperBlockEntity = (HupperBlockEntity)to;
          if (!hopperBlockEntity.isDisabled()) {
            int k = 0;
            if (from instanceof HupperBlockEntity) {
              HupperBlockEntity hopperBlockEntity2 = (HupperBlockEntity)from;
  ...
  private Inventory getOutputInventory() {
    Direction facing = (Direction)this.getCachedState().get(HupperBlock.FACING);
    Direction direction = facing == Direction.DOWN ? Direction.UP : facing;
